...
export default async function SettingsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // 🎨 BIOME CONFIGURATION
  // This helps us render the selector cards efficiently
  const biomes = [
    { 
      id: "plains", 
      name: "Plains", 
      bg: "bg-[#5b8731]", 
      border: "border-[#3e6826]", 
      text: "text-[#aaffaa]",
      desc: "Classic green grass & blue skies." 
    },
    { 
      id: "cherry", 
      name: "Cherry Grove", 
      bg: "bg-[#f7afce]", 
      border: "border-[#eb6eb0]", 
      text: "text-[#ffeef5]",
      desc: "Pink petals & dark wood." 
    },
    { 
      id: "ocean", 
      name: "Ocean Monument", 
      ...
      text: "text-[#aaffff]",
      desc: "Prismarine blue & sea lanterns." 
    },
    { 
      id: "spruce", 
      name: "Spruce Forest", 
      bg: "bg-[#3b2a1e]", 
      border: "border-[#26361c]", 
      text: "text-[#6c8c54]",
      desc: "Podzol dirt & tall pines." 
    },
    { 
      id: "cave", 
      name: "Lush Cave", 
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white font-sans p-6 md:p-12 relative overflow-hidden">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none opacity-20" 
           style={{ backgroundColor: "#0f0f0f", backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noiseFilter'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.9' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noiseFilter)' opacity='0.05'/%3E%3C/svg%3E")` }}>
      </div>
      ...
      <div className="max-w-4xl mx-auto space-y-12 relative z-10">
        ...
        <section className="bg-[#1a1a1a] border-4 border-[#555] p-6 shadow-[8px_8px_0_#000]">
          <h2 className="text-xl font-minecraft text-[#ccc] mb-6 flex items-center gap-2"><span className="text-[#888]">#</span> General</h2>
          <form action={renameWorld} className="flex flex-col md:flex-row gap-4 items-end">
            ...
            <div className="flex-1 w-full">
              ...
              <input name="newName" defaultValue={world.name} required className="w-full bg-[#111] border-2 border-[#333] p-3 text-white focus:outline-none focus:border-[#00aaaa] transition-colors font-minecraft" />
            </div>
            <button className="w-full md:w-auto bg-[#ccc] hover:bg-white text-black font-minecraft font-bold px-6 py-3 border-b-4 border-[#888] active:border-b-0 active:translate-y-1 active:mt-1 transition-all">SAVE</button>
          </form>
        </section>

        {/* 🌲 SECTION 2: Biome Selector (NEW!) */}
        <section className="bg-[#1a1a1a] border-4 border-[#555] p-6 shadow-[8px_8px_0_#000]">
          <h2 className="text-xl font-minecraft text-[#ccc] mb-6 flex items-center gap-2"><span className="text-[#888]">#</span> World Theme</h2>
              ...
              {biomes.map((b) => (
                <label key={b.id} className="cursor-pointer group relative">
                  ...
                  <div className={`h-32 ${b.bg} border-4 ${b.border} peer-checked:border-white peer-checked:shadow-[0_0_15px_white] transition-all flex flex-col items-center justify-center p-2 relative overflow-hidden group-hover:-translate-y-1`}>
                    ...
                    <span className={`font-minecraft text-lg drop-shadow-md relative z-10 ${b.text}`}>{b.name}</span>
        ...
        <section className="bg-[#0a1a1a] border-4 border-[#00aaaa] p-6 shadow-[8px_8px_0_#000]">
          <div className="flex justify-between items-center mb-6 border-b-2 border-[#00aaaa]/30 pb-4">
            <h2 className="text-xl font-minecraft text-[#00aaaa] flex items-center gap-2"><span>👥</span> Members</h2>
            <Link href={`/dashboard/invite/${worldId}`}>
               <button className="text-xs bg-[#00aaaa]/10 text-[#00aaaa] border border-[#00aaaa] px-3 py-2 font-minecraft hover:bg-[#00aaaa] hover:text-black transition-colors">+ INVITE NEW</button>
            </Link>
          </div>
                ...
                <div key={member.id} className="flex justify-between items-center p-4 bg-black/40 border-2 border-[#00aaaa]/30">
                  ...
                  {!isMe && (
                    <form action={kickMember}>
                      ...
                      <button className="text-[#aa0000] hover:text-[#ff5555] text-xs font-minecraft border-b-2 border-transparent hover:border-[#ff5555] transition-all pb-1">[ KICK ]</button>
                    </form>
                  )}
                </div>
              );
        ...
        <section className="bg-[#1a0505] border-4 border-[#aa0000] p-6 shadow-[8px_8px_0_#000] opacity-80 hover:opacity-100 transition-opacity">
          <h2 className="text-xl font-minecraft text-[#ff5555] mb-2 flex items-center gap-2"><span>🧨</span> Danger Zone</h2>
          <p className="text-[#aa5555] font-minecraft text-xs mb-6">Deleting a world is permanent. All chunks will be lost.</p>
